...
-record(room_info, {cards, people, counter, timer, active_position, round_over_position, 
...
-record(gambler, {pid, uid, state=waiting_next_game, has_raised=false, title=name, buyin, hand=[]}).
...
init_room() ->
	#room_info{cards = [], people = [], counter = 0, timer = undefined, one_v_one = false,  state = ready, blind_bet = 10,
	 board = [], pot = 0, high_bet = 0, dealer_position = 1, active_position = 1, round_over_position = 1,
...
leave_room(Room_info, Player_game_info, Player) ->
		...
		_ ->
			case Player_game_info#game_info.position of
				...
				Position ->
					...
					dm_room:broadcast(People, player_msg_install(room_state_changed, Room_info2)),
					Room_info2
			end
	end.
...
info2client(Player_info, Game_info) ->
	...
	Json 	  = dm_protocol:json_new([{uid,Uid},{money,Money},{counter,Counter},{ovo,One_v_one},{state,State},{blind,Blind_bet},{board,Board},{pot,Pot},
...
begin_game(Room_info) ->
	Room_info2 = reset_counter(prepare_active(change_game_state(deal_cards(shuffle_cards(prepare_blind(prepare_dealer(Room_info))), 2), pre_flop))),
		...
		_ -> 
			...
			Fun1                    =  	fun (G, {[H|T], Bets}) ->
											case {G, H} of
												...
												_ ->
													{gambler_blind(G, name, 0), {T, [0| Bets]}}
											end
										end, 
...
prepare_active(Room_info) ->
	...
	case {One_v_one, State} of
		{true, pre_flop} ->
			Room_info#room_info{active_position = Dealer_position, round_over_position = Dealer_position};
		{false, pre_flop} ->
			...
			Room_info#room_info{active_position = Next_active_position, round_over_position = Next_active_position};
		_->
...
deal_cards(Room_info, 0) ->
	Room_info;
deal_cards(Room_info, Num) ->
	...
	deal_cards(Room_info#room_info{cards=Cards1, gamblers=Gamblers1}, Num-1).
